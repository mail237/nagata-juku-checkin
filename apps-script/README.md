...
| `SPREADSHEET_ID` | スプレッドシートURLの `/d/` と `/edit/` のあいだの文字列。**スクリプトをそのブックに紐づけて作っている場合は空でも動く**ことが多いですが、未紐づけのスタンドアロンなら必須です |
...
- **生徒マスタ** 1行目の見出しは **A〜F の6列**（最後の **F＝学年**）。マスタの学年が、記録時に **入退室ログのF列**にコピーされます。
- **入退室ログ** も **A〜F**（Aタイムスタンプ、B生徒ID、C氏名、D種別、E送信ステータス、**F学年**）。

コードを直したあとは、**デプロイ → デプロイを管理 → 既存ウェブアプリの「編集（鉛筆）」→ バージョン「新バージョン」→ デプロイ** で再デプロイしないと、ブラウザや Vercel から呼んでも古いコードのままです。
