...
3. **1行目**に、次の5列の見出しを入れます（A〜E）。

   | A列 | B列 | C列 | D列 | E列 |
   |-----|-----|-----|-----|-----|
   | 生徒ID | 生徒氏名 | 保護者メールアドレス | QRコード値 | 備考 |
...
6. **入退室ログ** の **1行目**に、次の5列の見出しを入れます。

   | A列 | B列 | C列 | D列 | E列 |
   |-----|-----|-----|-----|-----|
   | タイムスタンプ | 生徒ID | 生徒氏名 | 種別 | 送信ステータス |
...
3. **Deploy** します。表示された **https://～** がタブレット用のURLです（本番は **HTTPS** 推奨）。

---

## 10. うまくいかないとき

- **記録されない・生徒が見つからない**: 学年・名前で選んだか確認する／QR利用時は生徒マスタ **D列（QRコード値）** が、貼った文字列と **完全一致** か確認する。
- **スプレッドシートエラー**: シート名が **`生徒マスタ`** と **`入退室ログ`** と完全一致か、サービスアカウントに共有しているか確認する。
